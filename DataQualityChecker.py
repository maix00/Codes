...
本模块定义了一个用于数据质量检查和处理的类 DataQualityChecker，适用于带有 symbol（如股票、期货代码）和时间字段的 pandas DataFrame 数据。其主要功能包括：
1. 数据质量检测：
    - 检查数值型列中的零值序列（如连续为零的区段），并根据其位置（如开头、结尾、全部、过长等）进行分类标记。
    - 检查时间序列中的缺失数据（基于最小时间间隔推断应有的时间点），并记录缺失区段。
    - 支持异常值检测（如 MAD、箱型等，部分功能预留）。
2. 问题记录与解决方案分配：
    - 检测到的问题会被详细记录在 issues_df DataFrame 中，包括问题类型、涉及的 symbol、列名、起止行、起止时间、详细描述等。
    - 根据问题类型自动分配推荐的处理方案（如线性插值、删除行、报错等），用户可自定义处理函数。
3. 数据处理与导出：
    - 可根据检测结果自动对原始数据进行修复处理，生成处理后的 DataFrame。
    - 支持将处理结果或问题记录导出为 csv、excel、parquet 等格式。
输入定义：
- df: pandas.DataFrame，必须包含 symbol 和时间字段（如 time、day、date），以及若干数值型列。
- columns: 可选，指定需要检查的列名列表。
- window_size_multiple: 控制零值序列判定为“过长”的窗口倍数。
- column_mapping: 可选，支持自定义 symbol 和 time 字段名的映射。
输出定义：
- issues_df: 检测到的所有数据质量问题的详细记录 DataFrame。
- processed_df: 经过自动修复处理后的 DataFrame，可按需导出。
主要功能：
- 自动识别并记录数据中的零值序列、缺失值、异常值等问题。
- 针对不同问题类型分配推荐的自动修复方案，支持自定义扩展。
- 支持分组（按 symbol）和分段处理，适合金融等时序数据场景。
- 提供详细的日志和导出功能，便于数据质量追踪和后续分析。
适用场景：
- 金融行情、传感器数据等需要高质量时序数据的场景。
- 需要批量检测和修复数据缺陷，提高数据可靠性的应用。
'''
