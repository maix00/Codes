...
class ContractRollover:
    ...
    def _calculate_key_timepoints(self):
        ...
        if not self.old_contract_old_data.empty and 'datetime' in self.old_contract_old_data.columns:
            self.old_contract_end_datetime = self.old_contract_old_data['datetime'].max()
            # 旧合约结束日期
        ...
        if (not self.new_contract_new_data.empty and 'datetime' in self.new_contract_new_data.columns and 
            self.old_contract_end_date):
            new_dates = pd.to_datetime(self.new_contract_new_data['datetime']).dt.date
            # 找到第一个与旧合约结束日期不同的日期
        ...
        elif not self.new_contract_new_data.empty and 'datetime' in self.new_contract_new_data.columns:
            # 如果没有旧合约结束日期，则直接使用新合约第一条数据的日期
            self.new_contract_start_date = pd.to_datetime(self.new_contract_new_data['datetime']).iloc[0].date()
        else:
    ...
    def get_data_summary(self) -> Dict[str, Any]:
        ...
        return {
            'old_contract_old_data': {
                ...
                'time_range': (self.old_contract_old_data['datetime'].min(), self.old_contract_old_data['datetime'].max()) if not self.old_contract_old_data.empty else (None, None),
            ...
            'old_contract_new_data': {
                ...
                'time_range': (self.old_contract_new_data['datetime'].min(), self.old_contract_new_data['datetime'].max()) if not self.old_contract_new_data.empty else (None, None),
            ...
            'new_contract_old_data': {
                ...
                'time_range': (self.new_contract_old_data['datetime'].min(), self.new_contract_old_data['datetime'].max()) if not self.new_contract_old_data.empty else (None, None),
            ...
            'new_contract_new_data': {
                ...
                'time_range': (self.new_contract_new_data['datetime'].min(), self.new_contract_new_data['datetime'].max()) if not self.new_contract_new_data.empty else (None, None),
